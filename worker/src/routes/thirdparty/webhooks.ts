...
import type { GatewayWebhookCreateRequest } from "../../gateway-types.ts";
...
export function thirdpartyWebhookRoutes(): Hono<PrivateEnv> {
  ...
  routes.get("/webhooks", userAuth, async (c) => {
    const user = c.var.user!;
    const db = c.var.db;
    const webhooks = await db.listGatewayWebhooks(user.id);
    return c.json(
      webhooks.map((w) => ({
        id: w.id,
        url: w.url,
  ...
  routes.post("/webhooks", userAuth, async (c) => {
    ...
    const id = newId();
    await db.createGatewayWebhook(id, user.id, body.url, body.event);

    return c.json({ id, url: body.url, event: body.event }, 201);
  });

  // DELETE /3rdparty/v1/webhooks/:id -- delete a webhook
  routes.delete("/webhooks/:id", userAuth, async (c) => {
    const db = c.var.db;
    const id = c.req.param("id");
    const wh = await db.getGatewayWebhook(id);
    if (!wh) {
      return c.json({ error: "Webhook not found" }, 404);
    }
    await db.deleteGatewayWebhook(id);

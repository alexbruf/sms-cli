...
export function mobileMessageRoutes(): Hono<PrivateEnv> {
  ...
  routes.patch("/message", deviceAuth, async (c) => {
    ...
    for (const update of body) {
      const msg = db.getGatewayMessage(update.id);
      if (!msg) continue;

      // Update overall message state
      db.updateGatewayMessageState(update.id, update.state, device.id);

      // Update per-recipient states
      if (update.recipients) {
        for (const r of update.recipients) {
          db.updateRecipientState(
            update.id,
            r.phoneNumber,
            r.state,
            r.error
